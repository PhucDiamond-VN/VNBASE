...
stock SM(playerid, SM_Type:type, const message[], OPEN_MP_TAGS:...){
		...
		foreach(new i:Player){
			if(IsPlayerAdmin(i)){
				switch(type){
					...
					case sm_error:return SendClientMessage(i, -1, va_return("{f74c43}[X] {ffffff}%s", message), ___(3));
				}
			}
		}
		return 1;
	...
	switch(type){
		...
		case sm_error:return SendClientMessage(playerid, -1, va_return("{f74c43}[X] {ffffff}%s", message), ___(3));
	}
	return 1;
}